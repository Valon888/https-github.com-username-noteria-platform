...
// ==========================================
// KONTROLLO ROLIN - VETËM ADMIN DHE NOTARY
// ==========================================
$user_roli = $_SESSION['roli'] ?? 'user';
if ($user_roli === 'user') {
    // Përdoruesit e thjeshtë nuk mund të hyjnë në dashboard
    ...
    ORDER BY r.date DESC, r.time DESC");
        ...
        <?php else: ?>
            <div class="admin-section">
                ...
                try {
                    $stmt = $pdo->query("SELECT z.id, z.emri AS zyra_emri FROM zyrat z ORDER BY z.id");
                    $zyrat_list = $stmt->fetchAll();
                    foreach ($zyrat_list as $z) {
                        echo "<h3>Zyra: " . htmlspecialchars($z['zyra_emri']) . "</h3><ul>";
                        // Merr përdoruesit (pa filtrimin sipas zyra_id pasi nuk ekziston)
                        $stmt2 = $pdo->query("SELECT u.id, u.emri, u.mbiemri, u.email FROM users u ORDER BY u.emri");
                    ...
                    error_log("Users by zyra query error: " . $e->getMessage());
                }
                ?>
            </div>
            <div class="admin-section">
...
<td>";
    ...
    // Kontrollo nëse tabela fatura ekziston
    $total_pagesa = 0;
    try {
        $total_pagesa = $pdo->query("SELECT COUNT(*) FROM fatura")->fetchColumn();
    } catch (PDOException $e) {
        // Tabela fatura nuk ekziston - set në 0
        error_log("Notice: fatura table doesn't exist - " . $e->getMessage());
    }
    ?>
    <ul>
        ...
        <div id="video-call-warning" style="display:none;margin-bottom:16px;background:#ffeaea;color:#d32f2f;padding:12px;border-radius:8px;font-weight:600;text-align:center;">
            ...
            <br>
            <a id="video-call-link" href="https://meet.jit.si/noteria_<?php echo $user_id; ?>" target="_blank" style="display:inline-block;margin-top:10px;background:#388e3c;color:#fff;padding:8px 18px;border-radius:8px;text-decoration:none;">
                Hyni në video thirrje
            </a>
        </div>
        <div class="logout">
...
<!-- Seksioni i të drejtave autoriale dhe GitHub -->
<footer style="text-align:center; margin-top:18px; color:#888; font-size:1rem;">
